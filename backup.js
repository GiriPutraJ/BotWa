...
if (msg.body === '.sgt') {
    try {
        const axios = require('axios');
        const { data } = await axios.get('https://www.growtopiagame.com/detail');
        console.log(data); // DEBUG: Tampilkan isi response

        const players = data.online_user;
        const status = data.status === 1 ? '🟢 *Online*' : '🔴 *Offline*';
        const update = new Date(data.last_world_update * 1000).toLocaleString('id-ID');
...
============================
        ...
        console.error(err);
        msg.reply('❌ Gagal mengambil data dari server Growtopia.');
    }
}
